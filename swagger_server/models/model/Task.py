...
class TaskTable(object):
    ...
    def get_published_task(user_id):
        ...
        sql = (
            ...
            " WHERE task.publish_id={user_id} AND user.user_id=task.publish_id".format(
                user_id=user_id
            )
        )
    ...
    def abort_task(task_id):
        ...
        放弃任务
        """
        sql = "UPDATE task SET state='F' WHERE task_id='{task_id}' ".format(
            task_id=task_id
        )
    ...
    def get_task_part_num(task_id):
        sql = "SELECT COUNT(*) as number FROM user_has_task WHERE task_task_id={task_id};".format(
            task_id=task_id
        )
    ...
    def get_task_participants(task_id):
        ...
        """
        sql = (
            "SELECT user.user_id, user.photo, user.nickname, cs.phone as phone "
            ...
            ") as cs, "
            "user "
            " WHERE u.user_user_id=user.user_id and cs.id=user.user_id;".format(
                id=task_id
            )
        )
    ...
    def get_task_intro(task_id):
        sql = "SELECT  task_intro From task  " "Where task_id={task_id}".format(
            task_id=task_id
        )
        result = Database.query(sql)
        return result
    ...
    def append_task_intro(task_id, content):
        content = "\n" + content
        sql = (
            "UPDATE task SET task_intro=CONCAT(task_intro,'{content}')  "
            "WHERE task_id={task_id} ".format(content=content, task_id=task_id)
        )
    ...
    def commit_job(user_id, task_id, file):
        ...
        """
        sql = (
            "INSERT INTO user_task_job(task_task_id, user_user_id, job_path, unload_time)  "
            " VALUES('{task_id}','{user_id}','{job_path}','{unload_time}') ".format(
                task_id=task_id,
                user_id=user_id,
                job_path=file,
        ...
        if isinstance(result, Exception):
            return result
        sql = (
            "UPDATE user_has_task as ut SET ut.isagree='W' WHERE  ut.task_task_id="
            "{task_id} AND ut.user_user_id={user_id}".format(
    ...
    def agree_job(participant_id, task_id, agree=True):
        ...
        """
        sql = (
            "UPDATE user_has_task as uh SET isagree='{agree}' "
    ...
    def participate_task(user_id, task_id):
        ...
        """
        sql = (
            "INSERT INTO user_has_task(user_user_id, task_task_id, task_audit_administrator_audit_id, isagree) "
            "VALUES({user_id}, {task_id}, '227', 'U')".format(
    ...
    def get_task_jobs(task_id):
        ...
        sql = (
            "SELECT jobs.user_user_id as user_id ,jobs.job_path as job  "
    ...
    def get_tasks(task_type="all", begin=0, end=100):
        if task_type == "all":
            ...
            # 倒序
            sql = (
                "SELECT task.*, user.photo as icon FROM task, user WHERE state='S' AND user.user_id=task.publish_id ORDER BY task.task_id DESC LIMIT %d OFFSET %d;"
                % (end - begin, begin)
            )
        elif task_type == "begin":
            # 倒序
            sql = (
                "SELECT task.*, user.photo as icon FROM task, user WHERE state='B' AND user.user_id=task.publish_id ORDER BY task.task_id DESC LIMIT %d OFFSET %d;"
                % (end - begin, begin)
            )
        else:
    ...
    def audit(task_id, audit):
        ...
        sql = "UPDATE task SET state= '%s'WHERE task_id=%d;" % (audit, task_id)
        print(sql)
...
def get_task_detail(task_id, user_id=None):
    ...
    sql2 = (
        ...
        " UNION ALL "
        " select phone_number, user_stu_id as id from stu_identity where user_user_id={id} ) as a "
        " WHERE u.user_id={id};".format(id=id_)
    )
    ...
    sql3 = "SELECT isagree FROM user_has_task as ut  WHERE ut.user_user_id = {user_id} AND ut.task_task_id={task_id} ".format(
        user_id=user_id, task_id=task_id
    )

    job = Database.query(sql3, fetchone=True)

    return {"task": task, "publisher": publisher, "job": job}
...
def create_task(**kwargs):
    ...
    sql = (
        "INSERT INTO task(title, type, publish_id, state, wjx_id, task_intro, max_num, participants_num, money, "
        "release_time, sign_start_time, sign_end_time, audit_administrator_audit_id) "
        "VALUES ('{title}', '{type_}', '{publish_id}', '{state}', '{wjx_id}', '{task_intro}',"
        " '{max_num}', '{participants_num}', '{money}', '{release_time}', '{sign_start_time}', '{sign_end_time}', "
        "'{audit_administrator_audit_id}'); ".format(**kwargs)
    )
    return Database.execute(sql, response=True)
