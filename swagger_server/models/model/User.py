...
class UserTable(object):
    ...
    @staticmethod
    def query_user(unionid_=None, user_id=None):
        """
            query_user by user_id or unionid

        """
        if unionid_ is not None:
            sql = "SELECT * " "FROM user " "WHERE wechat_id = '{id}'".format(id=unionid_)
        else:
            sql = "SELECT * " " FROM user " " WHERE user_id = '{id}'".format(id=user_id)
    ...
    def load_detail_user_id(self, user_id=None, identity=None):
        ...
        if identity is "S":
            ...
            查询student_identity
            """
            sql = "SELECT * FROM stu_identity WHERE user_user_id = '{id}'".format(
                id=user_id
            )
            ...
            """
            sql = "SELECT * FROM com_identity WHERE user_user_id = '{id}'".format(
                id=user_id
            )
        else:
            """
    ...
    def create_new_user(self, unionid, nickname="nick", photo=""):
        ...
        sql = (
            "INSERT INTO user(wechat_id, nickname ,identity, isprove, photo,intro, create_date, credit, balance) \
    ...
    @staticmethod
    def update_info(unionid, **kwargs):
        """
        ...
        query_sql = "SELECT * FROM user WHERE wechat_id='{unionid}'".format(
            unionid=unionid
        )
    ...
    def prove(
        ...
        if identity is "S":
            ...
            if info is None or len(info) is 0:
                sql = (
                    "INSERT INTO {stu_identity}(college, school,  name, phone_number,student_num, email, gender,prove, state_prove, user_user_id)\
            ...
            if info is None or len(info) is 0:
                sql = (
                    "INSERT INTO {com_identity}(company,  name, phone_number,job_num, email, gender,prove, state_prove, user_user_id)"
    ...
    @staticmethod
    def update_detail(user_id, identity, **kwargs):
        """
        ...
        if identity is "S":
            ...
            SET ".format(
                stu_identity=UserTable.stu_identity
            )
            for key, value in kwargs.items():
        ...
        query_sql = "SELECT * FROM user WHERE user_id = '{user_id}'".format(
            user_id=user_id
        )
        user = Database.query(query_sql, fetchone=True)
        return user
    ...
    def specific_user_count(gender="all", identity="S"):
        sql = (
            ...
            + "FROM {} as user ".format(
                "stu_identity" if identity == "S" else "com_identity"
            )
    ...
    def get_users(user_type="all", begin=0, end=100):
        if user_type == "all":
            ...
            sql = "SELECT * FROM user WHERE user.isprove='W' LIMIT %d OFFSET %d;" % (
                end - begin,
                begin,
            )
        else:
    ...
    def audit(user_id, identity, audit):
        ...
        sql = "UPDATE user SET user.isprove='%s' WHERE user.user_id = %d;" % (
            audit,
            user_id,
        )
        sql_ = "UPDATE %s SET state_prove='%s' WHERE user_user_id = %d;" % (
            "com_identity" if identity == "C" else "stu_identity",
            audit,
            user_id,
        )
        print(sql, sql_)
