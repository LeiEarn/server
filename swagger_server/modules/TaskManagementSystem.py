...
from .accessControlSystem import AccessControlSystem as AccessControl
from swagger_server.utils.utils import load_data, dump_data


def add_info(task_id, content):
    """
    发布者：增加任务说明
    """
...
def agree_job(task_id, user_id, state):
    ...
    if state == "agree":
        ...
        agree = False
    result = Task.taskTable.agree_job(
        participant_id=user_id, task_id=task_id, agree=agree
    )
...
class TaskManagementSystem(object):
    ...
    @AccessControl.owner_required(
        user_args="user_id", identity_error=("error", "identity error")
    )
    ...
    def get_task_accepter(self, task_id):
        """
        获取任务的参与者信息

        """
        acceptors = Task.taskTable.get_task_participants(task_id=task_id)
        return acceptors
    ...
    @staticmethod
    def get_related_tasks(userId, Type):
        if Type == "acceptment":
    ...
    def get_accepted_tasks(self, user_id):
        ...
        接收者：获取接受的任务
        """
        Task.taskTable.get_accepted_task(user_id=user_id)
    ...
    def commit_job(self, user_id, task_id, job):
        ...
        """
        pics = job.files
        materials = {"remarks": job.remarks, "pics": pics}
        file_name = dump_data(materials)

        result = Task.taskTable.commit_job(
            user_id=user_id, task_id=task_id, file=file_name
        )
        if isinstance(result, Exception):
            return "error", "提交工作失败"
        else:
    ...
    def get_tasks(task_type="all", page=0):
        return Task.taskTable.get_tasks(
            task_type=task_type, begin=page * 100, end=(page + 1) * 100
        )
...
def commit_task(task):
    ...
    payment = user.table.update_info(
        user.wechat_id, balance=user.balance - task.money
    )
